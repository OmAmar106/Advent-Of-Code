...
def solve():
    ...
    for i in range(len(L)):
        L1 = L[i].split(' ')
        L1.pop(0)
        elems = list(map(int,L1[-1].strip()[1:-1].split(',')))
        L1.pop()
        
        # minimize summation of 1*xi
        # given summation of 1*xi if j is present in i th element <= elemsj
        A = [[0]*len(L1) for i in range(len(elems))]
        ...
        val = (linprog([1]*len(A[0]),A_eq=A,b_eq=elems,integrality=True))
        try:
            fans += int(val.fun)
        except:
            print(val)
