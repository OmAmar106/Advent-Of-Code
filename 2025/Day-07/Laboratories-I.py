...
def solve():
    ...
    visited[L[0].index('S')] = True
    ans = 0

    for i in range(1,len(L)):
        newvisit = [False]*len(L[0])
        for j in range(len(L[0])):
            if L[i][j]=='^' and visited[j]:
                if j-1>=0:newvisit[j-1] = True
                if j+1<len((L[0])):newvisit[j+1] = True
                ans += 1
            elif L[i][j]=='.' and visited[j]:
                newvisit[j] = True
        visited = newvisit
